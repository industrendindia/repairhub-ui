...
const workItemOptions: WorkItemOption[] = [
  {
    ...
    description: "Motor rewinding and bush change.",
    defaultPrice: 600,
    warrantyDays: 90,
  },
  {
    ...
    description: "Capacitor replacement and cleaning.",
    defaultPrice: 300,
    warrantyDays: 60,
  },
  {
  ...
  },
  {
    ...
    itemName: "Iron box",
    category: "Home appliance",
    description: "Heating element and wiring check.",
    defaultPrice: 250,
    warrantyDays: 30,
  },
];
...
async function createTemporaryBillLink(payload: { billNumber: string; customerMobile: string }) {
  const hasMobile = payload.customerMobile.trim().length > 0;

  await httpClient.post("/bills/print-links", {
    billNumber: payload.billNumber,
    customerMobile: payload.customerMobile,
    smsOptedIn: hasMobile,
    whatsappOptedIn: hasMobile,
  });
}
      ...
      <div className="grid grid-cols-[24mm_1fr_72mm] gap-2 border-b border-[#0b2a66] pb-1.5">
        ...
        <div className="text-[11px] font-bold">
          ...
          <div className="grid grid-cols-[31mm_3mm_1fr] gap-y-1">
            ...
            <span>:</span>
            <span>{formatDeliveryDateTime(expectedDelivery)}</span>
          </div>
        </div>
      </div>
      ...
      <table className="mt-1 w-full table-fixed border-collapse text-[10px]">
        <thead>
          <tr className="bg-[#05245f] text-white">
            ...
            <th className="w-[30mm] border border-slate-400 py-1">Item</th>
            <th className="border border-slate-400 py-1">Description / Work Done</th>
            <th className="w-[34mm] border border-slate-400 py-1">Photo</th>
            <th className="w-[28mm] border border-slate-400 py-1">Price</th>
            <th className="w-[30mm] border border-slate-400 py-1">Actual Price</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr>
            <td colSpan={6} className="border border-slate-400 px-2 py-1 text-[10px] font-bold">
              Remarks / Fault Report : <span className="font-normal">{customer.notes || "Repair item received for service."}</span>
            </td>
          </tr>
        </tbody>
      </table>

      <div className="mt-1 grid grid-cols-[64mm_51mm_48mm_1fr] gap-1 text-[10px]">
        <div className="rounded border border-slate-400 p-2">
          <div className="grid grid-cols-[1fr_5mm_22mm] gap-y-1">
            ...
            <span>Discount</span>
            <span>Rs</span>
            <span className="text-right">{amount(discount)}</span>
            <span>Adjustment (-)</span>
            <span>Rs</span>
            <span className="text-right text-red-700">-{amount(adjustment)}</span>
          </div>
        ...
        <div className="rounded border border-slate-400 p-2">
          <div className="mx-auto mb-2 w-32 rounded bg-[#05245f] py-0.5 text-center font-black uppercase text-white">Repair Summary</div>
          <ul className="space-y-1">
            {printableItems.slice(0, 5).map((item, index) => (
              <li key={`${item.itemName}-${index}`} className="flex gap-1">
                <span className="font-black text-green-700">✓</span>
                <span>{item.description || `${item.itemName} checked`}</span>
              </li>
            ))}
          </ul>
        </div>

        <div className="rounded border border-slate-400 p-2 text-center">
          <div className="mx-auto mb-2 w-24 rounded bg-[#05245f] py-0.5 font-black uppercase text-white">Warranty</div>
          <div className="mt-5 text-3xl font-black text-[#05245f]">✓</div>
          <p className="mt-2 text-[12px] font-black text-[#05245f]">{firstWarranty || 0} DAYS</p>
          <p>(From Delivery Date)</p>
        </div>
      </div>

      <div className="mt-1 grid grid-cols-[116mm_1fr] gap-2 text-[9px]">
        ...
        <div className="rounded border border-slate-400 p-1.5 text-[6.5px] leading-tight">
          ...
          <ol className="list-decimal space-y-0.5 pl-3">
            <li>Warranty applies only to parts and work mentioned in this bill.</li>
            <li>Physical damage, water damage, misuse, or tampering voids warranty.</li>
            <li>Items must be collected within 30 days of service completion notice.</li>
            <li>Balance amount, if any, must be paid before delivery of repaired item.</li>
            <li>Original bill is required for warranty claim and delivery verification.</li>
            <li>Shop is not responsible for data loss, accessories, or old hidden faults.</li>
          </ol>
        </div>
      </div>
      {previewPhoto ? (
...
export function RepairIntakePage() {
  ...
  const handleBillSearch = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!query || isSearchingBills) return;
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <FormField label="Mobile" htmlFor="mobile">
                  <Input
                    ...
                    onChange={(event) => setCustomer((current) => ({ ...current, mobile: event.target.value }))}
                  />
                </FormField>
                ...
                <FormField label="Expected delivery" htmlFor="expectedDelivery">
                  <Input
                    ...
                    type="date"
                    min={today}
                    value={repairMeta.expectedDelivery}
                    onChange={(event) => setRepairMeta((current) => ({ ...current, expectedDelivery: event.target.value }))}
                  />
                </FormField>
                <FormField label="Priority" htmlFor="priority">
                  <Select
                    id="priority"
                    value={repairMeta.priority}
                    onChange={(event) => setRepairMeta((current) => ({ ...current, priority: event.target.value }))}
                  >
                ...
                <FormField className="md:col-span-2" label="Customer notes" htmlFor="notes">
                  <Textarea
                    id="notes"
            ...
            <section className="space-y-5">
              <div className="rounded-lg border bg-card p-5 shadow-soft">
                <div className="mb-5">
                  ...
                  <p className="text-sm text-muted-foreground">Add every item the customer brought, including serial number, pricing, and photos.</p>
                </div>
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <FormField label="Serial number" htmlFor="serialNo">
                    <Input
                      id="serialNo"
                      value={draftItem.serialNo}
                      onChange={(event) => setDraftItem((current) => ({ ...current, serialNo: event.target.value }))}
                    />
                  </FormField>
                  ...
                  <FormField label="Base price" htmlFor="basePrice" hint="Filled from work items table.">
                    <Input id="basePrice" type="number" value={draftItem.basePrice} readOnly />
                  </FormField>
                  <FormField label="Bill amount" htmlFor="actualPrice" hint="Editable if shop admin wants to change this item amount.">
                    <Input
                      id="actualPrice"
                      type="number"
                      min="0"
                  ...
                  <FormField label="Item reduction" htmlFor="itemDiscount" hint="Amount reduced by shop admin for this item.">
                    <Input
                      id="itemDiscount"
                      type="number"
                      min="0"
                      ...
                      onChange={(event) => setDraftItem((current) => ({ ...current, discount: Number(event.target.value) }))}
                    />
                  </FormField>
                  <FormField label="Warranty days" htmlFor="warrantyDays">
                    <Input
                      id="warrantyDays"
                      type="number"
                      min="0"
                      step="1"
                      value={draftItem.warrantyDays}
                      onChange={(event) => setDraftItem((current) => ({ ...current, warrantyDays: Number(event.target.value) }))}
                    />
                  </FormField>
            ...
            <section className="rounded-lg border bg-card p-5 shadow-soft">
              ...
              <div className="mt-5 space-y-3">
                {billSearchResults.length === 0 ? (
                  <p className="rounded-md border border-dashed px-4 py-8 text-center text-sm text-muted-foreground">
                    No bills loaded. Enter search details to find previous bills.
                  </p>
                ) : (
          ...
          {step === "billing" ? (
            <section className="rounded-lg border bg-card p-5 shadow-soft">
              <div className="mb-5">
                ...
                <p className="text-sm text-muted-foreground">Review item amounts and enter bill-level discount, adjustment reduction, and tax.</p>
              </div>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-3">
                <FormField label="Bill discount" htmlFor="billDiscount">
                  <Input
                    id="billDiscount"
                    type="number"
                    min="0"
                    ...
                    readOnly={Boolean(loadedBill)}
                    onChange={(event) => setBilling((current) => ({ ...current, discount: Number(event.target.value) }))}
                  />
                </FormField>
                <FormField label="Adjustment reduction" htmlFor="adjustment" hint="Use this to reduce the final bill amount.">
                  <Input
                    id="adjustment"
                    type="number"
                    min="0"
                    step="0.01"
                    value={billing.adjustment}
                    readOnly={Boolean(loadedBill)}
                    onChange={(event) => setBilling((current) => ({ ...current, adjustment: Number(event.target.value) }))}
                  />
                </FormField>
                <FormField label="Tax" htmlFor="tax">
                  <Input
                    ...
                    value={billing.tax}
                    readOnly={Boolean(loadedBill)}
                    onChange={(event) => setBilling((current) => ({ ...current, tax: Number(event.target.value) }))}
                  />
                </FormField>
              </div>
              <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
          ...
          {step === "payment" ? (
            <form className="rounded-lg border bg-card p-5 shadow-soft" onSubmit={completePayment}>
              <div className="mb-5">
                ...
                <p className="text-sm text-muted-foreground">Record payment mode, amount received, and reference details.</p>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <FormField label="Reference number" htmlFor="referenceNumber">
                  <Input
                    ...
                    onChange={(event) => setPayment((current) => ({ ...current, referenceNumber: event.target.value }))}
                  />
                </FormField>
          ...
          {step === "final" ? (
            <section className="print-root">
              ...
              <BillPrintLayout
                ...
                customer={customer}
                expectedDelivery={repairMeta.expectedDelivery}
                items={items}
                subtotal={subtotal}
