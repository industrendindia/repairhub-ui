...
<script>(()=>{const c=document.getElementById('pad'),box=c.parentElement,ctx=c.getContext('2d'),ok=document.getElementById('ok');let drawing=false,signed=false,last=null,scale=1,minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity;function size(){const r=box.getBoundingClientRect(),d=Math.min(window.devicePixelRatio||1,2),old=signed?c.toDataURL():null;scale=d;c.width=Math.round(r.width*d);c.height=Math.round(r.height*d);ctx.setTransform(d,0,0,d,0,0);ctx.lineWidth=5;ctx.lineCap='round';ctx.lineJoin='round';ctx.strokeStyle='#0f172a';if(old){const i=new Image();i.onload=()=>ctx.drawImage(i,0,0,r.width,r.height);i.src=old}}function point(e){const r=c.getBoundingClientRect();return{x:e.clientX-r.left,y:e.clientY-r.top}}function bounds(p){minX=Math.min(minX,p.x);minY=Math.min(minY,p.y);maxX=Math.max(maxX,p.x);maxY=Math.max(maxY,p.y)}c.addEventListener('pointerdown',e=>{drawing=true;last=point(e);bounds(last);c.setPointerCapture(e.pointerId)});c.addEventListener('pointermove',e=>{if(!drawing)return;const p=point(e);ctx.beginPath();ctx.moveTo(last.x,last.y);ctx.lineTo(p.x,p.y);ctx.stroke();bounds(p);last=p;signed=true;ok.disabled=false});c.addEventListener('pointerup',()=>drawing=false);c.addEventListener('pointercancel',()=>drawing=false);document.getElementById('clear').onclick=()=>{ctx.clearRect(0,0,c.width,c.height);signed=false;minX=minY=Infinity;maxX=maxY=-Infinity;ok.disabled=true};ok.onclick=()=>{if(!signed)return;const pad=14,x=Math.max(minX-pad,0),y=Math.max(minY-pad,0),w=Math.min(maxX+pad,c.width/scale)-x,h=Math.min(maxY+pad,c.height/scale)-y,out=document.createElement('canvas');out.width=Math.ceil(w*scale);out.height=Math.ceil(h*scale);out.getContext('2d').drawImage(c,x*scale,y*scale,w*scale,h*scale,0,0,out.width,out.height);window.opener.postMessage({type:'repairhub-signature',url:out.toDataURL('image/png')},window.location.origin);window.close()};window.addEventListener('resize',size);size()})()<\/script></body></html>`);
    ...
    <article className="half-a4-bill bg-white text-slate-950">
      ...
      <div className="mt-1 grid grid-cols-[92mm_1fr] gap-2 text-[10px]">
        <div className="grid grid-cols-2 rounded border border-slate-400">
          <div className="min-h-[22mm] border-r border-slate-400 p-1 text-center font-bold">
            Customer Signature
            {customer.signature?.url ? (
              <img src={customer.signature.url} alt="Customer signature" className="mx-auto mt-1 h-[14mm] max-w-36 object-contain" />
            ) : null}
            <div className={`mx-auto w-28 border-t border-slate-500 ${customer.signature?.url ? "mt-0.5" : "mt-8"}`} />
          </div>
          <div className="relative min-h-[22mm] p-1 text-center font-bold">
...
export function RepairIntakePage() {
  ...
  useEffect(() => {
    if (!isWorkItemDropdownOpen) return;
    const closeWorkItemDropdown = (event: MouseEvent | TouchEvent) => {
      if (!workItemDropdownRef.current?.contains(event.target as Node)) {
        setIsWorkItemDropdownOpen(false);
      }
    };
    document.addEventListener("mousedown", closeWorkItemDropdown);
    document.addEventListener("touchstart", closeWorkItemDropdown);
    return () => {
      document.removeEventListener("mousedown", closeWorkItemDropdown);
      document.removeEventListener("touchstart", closeWorkItemDropdown);
    };
  ...
  const selectWorkItem = (workItemId: string) => {
    ...
    setWorkItemSearch(selected?.itemName ?? "");
    setIsWorkItemDropdownOpen(false);
  };
                          ...
                          {filteredWorkItemOptions.length ? (
                            filteredWorkItemOptions.map((item, index) => (
                              ...
                              >
                                ...
                                <span className="shrink-0 text-muted-foreground">{currency.format(item.defaultPrice)}</span>
                              </button>
                            ))
                          ) : (
